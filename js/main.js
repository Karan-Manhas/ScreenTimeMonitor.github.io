...
function setupReminders() {
  ...
  document.getElementById("save-reminders").addEventListener("click", () => {
    ...
    localStorage.setItem(
      "reminderSettings",
      JSON.stringify({ interval, soundAlert, notificationTypes })
    );
...
function logHealthSymptoms() {
  document.getElementById("save-symptoms").addEventListener("click", () => {
    ...
    const severityEyeStrainElement = document.getElementById(
      "severity-eye-strain"
    );
    const severityHeadachesElement =
      document.getElementById("severity-headaches");
    const severityAnxietyElement = document.getElementById("severity-anxiety");

    const severity = {
      eyeStrain: severityEyeStrainElement
        ? severityEyeStrainElement.value
        : null,
      headaches: severityHeadachesElement
        ? severityHeadachesElement.value
        : null,
...
function weeklyReport() {
  ...
  const healthSymptoms =
    JSON.parse(localStorage.getItem("healthSymptoms")) || {};
  ...
  setTimeout(() => {
    ...
    reportDiv.innerHTML += `
            ...
            <p>Scheduled Interval Breaks(Minutes): ${
              reminderSettings.interval || "Not set"
            }</p>
            <p>Notification Types: ${
              (reminderSettings.notificationTypes || []).join(", ") || "None"
            }</p>
...
function sendEmailWithCsv() {
  ...
  var csvBreakFrequency = generateCsvData(
    breakFrequencyData,
    "Day,Break Frequency"
  );
